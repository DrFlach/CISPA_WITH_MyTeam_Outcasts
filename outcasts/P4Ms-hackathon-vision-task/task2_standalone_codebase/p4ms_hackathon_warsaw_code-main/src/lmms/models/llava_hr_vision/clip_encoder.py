...
from transformers import CLIPVisionModel, CLIPImageProcessor, CLIPVisionConfig, logging
...
def forward_embeddings(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
    ...
    if position_embeddings.shape[1] != embeddings.shape[1]:
        position_embeddings = resample_pos_embed(
            position_embeddings, embeddings.shape[1]
        )
