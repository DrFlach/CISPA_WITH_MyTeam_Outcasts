...
from src.lmms.models.llava_hr_vision.convnext_encoder import ConvNextVisionTower
from src.lmms.models.llava_hr_vision.clip_encoder import CLIPVisionTower
...
class MultiPathAlignModule(nn.Module):
    ...
    def forward(self, fast_feat, slow_feat):
        ...
        assert (
            slow_feat.shape[1] % fast_feat.shape[1] == 0
            or fast_feat.shape[1] % slow_feat.shape[1] == 0
        )
        if slow_feat.shape[1] < fast_feat.shape[1]:
            ...
            dst_size = int(math.sqrt(fast_feat.shape[1]))
            slow_feat = slow_feat.transpose(1, 2).view(b, c, src_size, src_size)
            slow_feat = F.avg_pool2d(
                slow_feat, src_size // dst_size, src_size // dst_size
            )
            slow_feat = slow_feat.view(b, c, -1).transpose(1, 2)
...
class S2FStitchAlignModuleV2(nn.Module):
    def __init__(self, fast_vision_dim, slow_vision_dim, zero_init=True):
        ...
        self.fast_conv = nn.Conv2d(
            fast_vision_dim, fast_vision_dim, 7, padding=3, groups=fast_vision_dim
        )
    ...
    def src2dst_align(self, src_feat, dst_feat):
        ...
        assert (
            src_feat.shape[1] % dst_feat.shape[1] == 0
            or dst_feat.shape[1] % src_feat.shape[1] == 0
        )
        if src_feat.shape[1] < dst_feat.shape[1]:
            ...
            dst_size = int(math.sqrt(dst_feat.shape[1]))
            src_feat = src_feat.transpose(1, 2).view(b, c, src_size, src_size)
            src_feat = F.avg_pool2d(
                src_feat, src_size // dst_size, src_size // dst_size
            )
            src_feat = src_feat.view(b, c, -1).transpose(1, 2)
    ...
    def forward(self, fast_feat, slow_feat):
        ...
        gate = self.gate(torch.cat([fast_feat, slow_feat_align], -1).mean(1)).unsqueeze(
            1
        )
...
class MultiPathCLIPVisionTower(nn.Module):
    def __init__(self, vision_tower, args, delay_load=False):
        ...
        self.slow_vision_tower = ConvNextVisionTower(
            args["vision_tower_slow"], args, delay_load=delay_load
        )
        args_ = deepcopy(args)
        # set a default image size
        args_["input_image_size"] = 336
        self.fast_vision_tower = CLIPVisionTower(
            vision_tower, args_, delay_load=delay_load
        )
        ...
        if self.enable_adapter:
            self.align_stages_latent = nn.ModuleList(
                [
                    S2FStitchAlignModuleV2(
                        self.fast_vision_tower.hidden_size,
    ...
    def forward_features(self, x):
        ...
        y = F.interpolate(
            x.float(),
            size=(fast_image_size, fast_image_size),
            mode="bilinear",
            align_corners=True,
        ).to(dtype=x.dtype)
        ...
        for blk in fast_blk[:n_blks]:
            if self.training:
                y = checkpoint(blk.__call__, y, None, None)[0]
            else:
                y = blk(y, None, None)[0]
        if self.enable_adapter:
