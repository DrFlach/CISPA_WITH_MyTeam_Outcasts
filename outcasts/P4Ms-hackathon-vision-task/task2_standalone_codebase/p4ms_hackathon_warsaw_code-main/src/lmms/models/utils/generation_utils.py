...
class GenerationMixin:
    ...
    def _get_generation_mode(
        ...
        if (
            ...
            or generation_config.prompt_lookup_num_tokens is not None
        ):
        ...
        while True:
            ...
            if sequential:
                ...
                next_hidden = torch.stack(
                    [all_last_hstates[i] for i in range(top_k)], dim=0
                )
                ...
                for layer in range(len(full_hidden_states)):
                    final_full_hstates[layer] = torch.stack(
                        [torch.squeeze(all_hstates[i][layer], 0) for i in range(top_k)],
                        dim=0,
                    )
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # if stopping_criteria(input_ids, scores):
            #     this_peer_finished = True
            stopping_result = stopping_criteria(input_ids, scores)
            if isinstance(stopping_result, torch.Tensor):
                stopping_result = stopping_result.all()
            if stopping_result:
                this_peer_finished = True
            if this_peer_finished and not synced_gpus:
                break
