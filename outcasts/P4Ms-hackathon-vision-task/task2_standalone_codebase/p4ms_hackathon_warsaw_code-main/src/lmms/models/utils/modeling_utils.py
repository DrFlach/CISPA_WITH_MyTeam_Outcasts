...
from src.lmms.models.utils.generation_utils import GenerationMixin
...
class PreTrainedModel(
    ...
    def _from_config(cls, config, **kwargs):
        ...
        use_eager = kwargs.pop("use_eager", False)
    ...
    def _autoset_attn_implementation(
        ...
        Automatically checks and dispatches to a default attention implementation. In order of priority:
            ...
            2. DEPRECATED: if use_eager is set to `True` and `flash_attn` is available, flash attention. (`LlamaFlashAttention` for example)
        ...
        if (
            ...
            if (
                ...
                raise ValueError(
                    f'Both attn_implementation="{config._attn_implementation}" and `use_eager=True` were used when loading the model, which are not compatible.'
                    ' We recommend to just use `attn_implementation="eager"` when loading the model.'
                )
            ...
            if (
                ...
                if cls._supports_eager:
                    message += ', `"attn_implementation=eager"` (implementation using flash attention 2)'
        ...
        if use_eager:
            logger.warning_once(
                'The model was loaded with use_eager=True, which is deprecated and may be removed in a future release. Please use `attn_implementation="eager"` instead.'
            )
            config._attn_implementation = "eager"

        if config._attn_implementation == "eager":
            cls._check_and_enable_eager(
                config,
                torch_dtype=torch_dtype,
                device_map=device_map,
            ...
            # use_eager takes priority over SDPA, hence SDPA treated in this elif.
    ...
    def _check_and_enable_eager(
        ...
        If all checks pass and `hard_check_only` is False, the method will set the config attribute `attn_implementation` to "eager" so that the model can initialize the correct attention module.
        """
        if not cls._supports_eager:
            raise ValueError(
        ...
        if torch_dtype is None:
            ...
            logger.warning_once(
                ...
                ' or load the model with the `torch_dtype` argument. Example: `model = AutoModel.from_pretrained("openai/whisper-tiny", attn_implementation="eager", torch_dtype=torch.float16)`'
            )
        ...
        **kwargs,
        ...
        Parameters:
            ...
            attn_implementation (`str`, *optional*):
                The attention implementation to use in the model (if relevant). Can be any of `"eager"` (manual implementation of the attention), `"sdpa"` (using [`F.scaled_dot_product_attention`](https://pytorch.org/docs/master/generated/torch.nn.functional.scaled_dot_product_attention.html)), or `"eager"` (using [Dao-AILab/flash-attention](https://github.com/Dao-AILab/flash-attention)). By default, if available, SDPA will be used for torch>=2.1.1. The default is otherwise the manual `"eager"` implementation.
