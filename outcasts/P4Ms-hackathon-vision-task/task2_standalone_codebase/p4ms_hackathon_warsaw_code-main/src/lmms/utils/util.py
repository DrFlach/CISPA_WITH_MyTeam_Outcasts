...
def prepare_sample(
    data: Union[torch.Tensor, Any], device, dtype=None
    ...
    if isinstance(data, Mapping):
        return type(data)(
            {k: prepare_sample(v, device, dtype) for k, v in data.items()}
        )
    elif isinstance(data, (tuple, list)):
        return type(data)(prepare_sample(v, device, dtype) for v in data)
